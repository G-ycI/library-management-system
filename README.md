...
一个基于 Next.js + Flask 的全栈图书管理系统，提供图书的增删改查、搜索筛选、分页浏览等功能。

## 项目介绍

本项目是一个现代化的图书管理系统，采用前后端分离架构。前端使用 Next.js 构建响应式用户界面，后端使用 Flask 提供 RESTful API 服务，数据存储采用 SQLite 数据库。系统支持图书信息的完整生命周期管理，包括新增、查询、修改、删除，以及按关键词搜索和分类筛选。
...
4. **网络异常**：前端API请求失败时显示错误提示，不影响页面基础展示
5. **空数据**：无图书数据时显示空状态提示
